...
public abstract class PlayerScreenHandlerMixin extends AbstractRecipeScreenHandler<CraftingInventory> {
    ...
    public void onContentChanged(Inventory inventory) {
        if (this.onServer) {
            ...
            for(i = 0; i < 2; ++i) {
                for(j = 0; j < 2; ++j) {
                    Item inputItem = ((PlayerScreenHandlerAccessor) this).getCraftingInput().getStack(j + i * 2).getItem();
//                    String inputItemName = Registry.ITEM.getId(inputItem).toString();
            ...
            if(item != BlockInit.FLINT_CRAFTING_TABLE.asItem() && count > 0 || AliveAndWellMain.day<=AliveAndWellMain.structureUnderDay && (name.equals("soulsweapons:boss_compass")|| name.equals("bosses_of_mass_destruction:void_lily"))){
                ...
                CraftingScreenHandler.updateResult(this, this.owner.getWorld(), this.owner, ((PlayerScreenHandlerAccessor) this).getCraftingInput(), this.craftingResult);
            }
        }
    }
}
